...
const NavArea = () => {
  ...
  const [mode, modeSet] = useState(theme);
  ...
  const setTheme = (mode: typeof theme) => {
    modeSet(mode);
    ...
    <>
      <ul className="ml-4 mt-2 hidden items-end lg:flex">
        {routes.map((route) => (
          <li
            key={route.name}
            className="mx-1 px-2 py-1 text-lg transition-colors hover:bg-primary-500 hover:bg-opacity-20"
          >
